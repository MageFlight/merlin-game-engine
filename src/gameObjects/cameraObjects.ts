...
export class Camera extends GameObject {
   ...
   */
  constructor(scrollMin: Vector2, scrollMax: Vector2, leftBound: number, rightBound: number, upBound: number, downBound: number, horizontalLock: boolean, verticalLock: boolean, initialCamera: boolean, name: string) {
    super(name);
  ...
  calculateScroll() {
    ...
    if (this.parent !== null && this.parent instanceof Sprite) {
      parentScreenPosition = new Vector2(this.parent.getGlobalPos().x - this.scrollPos.x, this.parent.getGlobalPos().y - this.scrollPos.y);
    } else {
    ...
    if (!this.horizontalLock) {
      if (parentScreenPosition.x > this.rightBound) this.scrollPos.x += parentScreenPosition.x - this.rightBound;
      if (parentScreenPosition.x < this.leftBound) this.scrollPos.x += parentScreenPosition.x - this.leftBound;
      this.scrollPos.x = Math.min(Math.max(this.scrollPos.x, this.scrollMin.x), this.scrollMax.x - Utils.GAME_WIDTH);
    }
    // y-axis
    if (!this.verticalLock) {
      ...
      if (parentScreenPosition.y < this.upBound) this.scrollPos.y += parentScreenPosition.y - this.upBound;
      if (parentScreenPosition.y > this.downBound) this.scrollPos.y += parentScreenPosition.y - this.downBound;
      log("beforeClamp: " + this.scrollPos.y);
      this.scrollPos.y = Math.min(Math.max(this.scrollPos.y, this.scrollMin.y), this.scrollMax.y)
